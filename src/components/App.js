...
class App extends Component {
  ...
  async loadBlockchainData() {
    ...
    if (networkData) {
      const dtube = new web3.eth.Contract(DVideo.abi, networkData.address);
      this.setState({ dvideo: dtube });
      ...
      this.setState({ currentHash: latest.hash });
      this.setState({ currentTitle: latest.title });
      this.setState({ loading: false });
    } else {
      window.alert("Contract was not deployed to the nerwork");
    }
